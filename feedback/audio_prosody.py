...
class AudioProsodyFeedback(FeedbackBackend):
    def __init__(self,
    ...
    def poll(self):
        ...
        if self.print_debug:
            print(
                f"[AUDIO] fb={fb} "
                f"valence={latest['valence']:+.4f} "
                f"reward={latest['reward_cont']:+.3f} "
                f"conf={latest['conf']:.2f}"
            )
